    ...
    <>
      ...
      <div className="adm-panel">
        ...
        <div className="adm-grid2" style={{ marginTop:'var(--s-4)' }}>
          ...
          <div className="field"><label>Sipariş miktarı (min)</label><input className="input" inputMode="numeric" value={p.minQty} onChange={e=>setP(s=>({...s,minQty:num(e.target.value)}))} /></div>
      ...
      <div className="adm-panel">
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <strong>Miktar Bazlı Fiyatlar (miktar → 1.000 adet fiyatı)</strong>
          <div style={{ display:'flex', gap:'.5rem' }}>
            <button className="adm-btn adm-btn--ghost" onClick={()=>setP(s=>{
              const std=[1000,2000,5000,10000,25000,50000,100000];
              const have=new Set(s.tiers.map(t=>t.minQty));
              const add=std.filter(q=>!have.has(q)).map(q=>({ minQty:q, ratePer1000Cents:s.basePriceCents, badgeDe:'', badgeEn:'', badgeFr:'', isActive:true }));
              return add.length ? ({ ...s, tiers:[...s.tiers, ...add].sort((a,b)=>a.minQty-b.minQty) }) : s;
            })}>Standart kademeleri ekle</button>
        ...
        <p className="muted" style={{ fontSize:'.82rem', margin:'.3rem 0 var(--s-3)' }}>Bu alanda her miktar kademesi için 1.000 adet fiyatını belirlersiniz. Örneğin: 1.000 adet → €269 / 1.000, 5.000 adet → €249 / 1.000. Toplam sipariş fiyatı sistem tarafından otomatik hesaplanır (salt-okunur). Her ürün bağımsızdır; özel adet en yakın alt kademeye yuvarlanır. Yaygın kademeler: 1.000 · 2.000 · 5.000 · 10.000 · 25.000 · 50.000 · 100.000.</p>
          ...
          <div style={{ display:'grid', gap:'.5rem' }}>
            {[...p.tiers].map((tt, i) => (
              <div key={i} className="adm-grid2" style={{ alignItems:'end', gridTemplateColumns:'1fr 1fr auto auto', gap:'.5rem' }}>
                <div className="field" style={{ margin:0 }}><label>Miktar (adet)</label><input className="input" inputMode="numeric" value={tt.minQty} onChange={e=>setP(s=>({...s, tiers:s.tiers.map((x,j)=>j===i?{...x,minQty:num(e.target.value)}:x)}))} /></div>
                <div className="field" style={{ margin:0 }}><label>1.000 adet fiyatı</label><MoneyInput cents={tt.ratePer1000Cents} onCents={c=>setP(s=>({...s, tiers:s.tiers.map((x,j)=>j===i?{...x,ratePer1000Cents:c ?? 0}:x)}))} /></div>
                <div className="field" style={{ margin:0 }}><label>Otomatik toplam</label><input className="input" readOnly value={`${((tt.ratePer1000Cents*(tt.minQty||0)/1000)/100).toFixed(0)} €`} /></div>
