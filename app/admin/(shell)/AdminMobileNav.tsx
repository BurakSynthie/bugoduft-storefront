...
import { ADMIN_NAV, ADMIN_NAV_SHEET } from '@/lib/admin/nav';
...
// Phase 6E-B2 §24: derive the mobile title from the single ADMIN_NAV source of truth
// instead of a hand-maintained partial list, so every real module (Koleksiyonlar,
// Çeviriler, SEO, Ayarlar, Kokular, Teklifler, ...) gets its real title and only a
// genuinely unknown route ever falls back to the generic "Yönetim".
const ALL_NAV_ITEMS = ADMIN_NAV.flatMap(g => g.items);
function titleFor(pathname: string): string {
  if (pathname === '/admin') return 'Panel';
  const matches = ALL_NAV_ITEMS.filter(it => it.href !== '/admin' && pathname.startsWith(it.href));
  matches.sort((a, b) => b.href.length - a.href.length);
  return matches[0]?.label ?? 'Yönetim';
}
