  ...
  return (
    <AccountShell locale={locale} active={`/${locale}/konto`} email={user.email}>
      ...
      {saved.length > 0 && (
        <div className="acct-block" style={{ alignItems:'stretch' }}>
          ...
          {saved.slice(0,5).map(s => (
            <div key={s.id} className="acct-order__meta"><span>{s.label ?? s.collectionCode ?? s.productCode ?? '—'}</span><span>{new Date(s.updatedAt).toLocaleDateString(locale)}</span></div>
          ))}
        </div>
      )}
      {quotes.length > 0 && (
      ...
      <div className="acct-block" style={{ alignItems:'stretch' }}>
        ...
        <Link className="btn btn--dark" href={configuratorPath(locale)} style={{ alignSelf:'flex-start' }}>{ACCOUNT_COPY[locale].reorder /* start new */ ? 'Jetzt gestalten' : 'Jetzt gestalten'}</Link>
      </div>
    </AccountShell>
  );
}
