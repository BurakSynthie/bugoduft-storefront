...
// DEPRECATED as of Phase 6E-B2 (§10, release blocker): this Storefront Cart API flow can
// only charge a FIXED catalog variant price and cannot represent BUGO's quantity-tiered
// authoritative total, so it charged the wrong amount. app/actions/checkout.ts now uses
// lib/shopify/draft-order.ts (Admin API Draft Order with an explicit price override)
// instead. Left in place, unused, only as a reference for the legacy `basic checkout,
// qty=1, fixed price` shape — do not wire this back into checkout without also solving
// the price-override problem it originally had.
