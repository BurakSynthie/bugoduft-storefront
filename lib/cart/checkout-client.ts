...
export async function checkoutCartItem(item: CartItem, fallbackErr: string): Promise<CheckoutResult> {
  ...
  if (item.filesPersisted) {
    ...
    // Server error strings (repositories/*, lib/shopify/draft-order.ts) are only ever
    // written in German — never show them to an en/fr customer; use the caller's
    // localized fallbackErr instead (raw message is still fine for de).
    return fin.ok ? { ok: true, url: fin.checkoutUrl } : { ok: false, message: item.locale === 'de' ? fin.message : fallbackErr };
  }
