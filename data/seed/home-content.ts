...
export function getHomeContent(locale: Locale): HomeExtra {
  // Inject the section-chrome seed so the storefront and the admin editor both have a
  // complete default set; a stored homepage_content doc overrides it via the merge in
  // repositories/homepage.ts.
  return { ...map[locale], sections: HOME_SECTIONS[locale] };
}
