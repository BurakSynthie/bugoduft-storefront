...
export default function QuoteForm({ locale, sec }: { locale: Locale; sec?: HomeSections }) {
  const c = t[locale];
  ...
  async function onSubmit() {
    ...
    if (res.ok) { setState('ok'); setCompany(''); setEmail(''); setQuantity(''); setMessage(''); try { window.bugoTrack?.('quote_submit'); } catch {} }
    ...
    <section className="section section--subtle" id="angebot">
      <Container>
        <div className="quote2">
          <div><SectionHeader eyebrow={s.quoteEye} title={s.quoteTitle} lede={s.quoteLede} /></div>
